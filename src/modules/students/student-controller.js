...
import {
  ...
  processGetStudents,
  processUpdateStudentSystemStatus
...
export const handleUpdateStudentSystemStatus = asyncHandler(async (req, res) => {
  const { schoolId } = req.user;
  const { id: userId } = req.params;
  const { hasSystemAccess } = req.body;
  const response = await processUpdateStudentSystemStatus({
    schoolId,
    hasSystemAccess,
    userId
  });
