...
export const processUpdateStudentSystemStatus = async (payload) => {
  const affectedRow = await updateStudentSystemStatus(payload);
  if (affectedRow <= 0) {
    throw new ApiError(500, 'Unable to update system access status');
  }
  return { message: 'Syste access status updated successfully' };
};
